define([
	...
	welcomePopup1.prototype.init = function(){
		...
		Services.InvestmentProfile.get(function(err, response){
	...
	welcomePopup1.prototype.submit = function(){
		...
		if( validFirstName 
			...
			Services.InvestmentProfile.update( params, function(err, response){
				...
				if( response.spHeader.success !== true ){
					...
					Services.Feedback.send({
						description: 'HTML First Use Submission Error: ' + jsonParams
					});
				}
			});
	...
	*/
	...
	welcomePopup2.prototype.init = function(){
		...
		// on init, get the existing investment profile, if any
		Services.InvestmentProfile.get(function(er, response){
			if( response && response.spHeader.success === true ){
				self.populateFirstUseForm(response.spData);
			}
		});
	...
	var welcomePopup = {
			...
			, loadUserVariant: function(variantID){
				variantID = variantID.replace('/welcomePopup/', '');
			...
			, close: function(disableRedirect){
				window.isWelcomePopupHidden = true;
